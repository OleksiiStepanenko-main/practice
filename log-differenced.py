...
for country in data.columns:
    ...
    # THE FIX: Apply LOGARITHMIC differencing (z_t = ln(y_t) - ln(y_{t-1}))
    df_series['Log_Diff'] = np.log(df_series['CO2']).diff()

    # Create lagged features based purely on the LOG DIFFERENCES
    for i in range(1, 4):
        df_series[f'Lag_{i}'] = df_series['Log_Diff'].shift(i)
    ...
    for target_year in years_to_predict:
        ...
        # Features are the lagged log diffs, target is the current log diff
        X_train = train_data[['Lag_1', 'Lag_2', 'Lag_3']]
        y_train = train_data['Log_Diff']
        X_test = test_data[['Lag_1', 'Lag_2', 'Lag_3']]

        model = DecisionTreeRegressor(max_depth=5, random_state=42)
        model.fit(X_train, y_train)

        # Predict the LOG DIFFERENCE for the target year
        pred_log_diff = model.predict(X_test)[0]

        # RECONSTRUCTION FIX: Actual Previous Year * exp(Predicted Log Diff)
        actual_previous_year = data.loc[target_year - 1, country]
        pred_absolute = actual_previous_year * np.exp(pred_log_diff)
    ...
    y_full = df_series['Log_Diff']

    final_model = DecisionTreeRegressor(max_depth=5, random_state=42)
    final_model.fit(X_full, y_full)

    # Lags for 2025 are the actual log differences from 2024, 2023, and 2022
    ...
    # Predict the 2025 log difference and apply the exponential reconstruction
    pred_log_diff_2025 = final_model.predict(lags_for_2025)[0]
    actual_2024_absolute = data.loc[2024, country]
    forecast_2025 = actual_2024_absolute * np.exp(pred_log_diff_2025)
...
for idx, country in enumerate(data.columns):
    ...
    ax.set_title(f"{country} CO2 Emissions Forecast (Log-Differenced Tree)")
    ax.set_xlabel("Year")
